 ...
 *   ./persk   (person management)
 *   ./newcat  (category management)
 *
 * Build all three binaries first:
 *   gcc persk-1.c  -o persk  $(pkg-config --cflags --libs gtk4)
 *   gcc newcat.c   -o newcat $(pkg-config --cflags --libs gtk4)
 */
...
{
    (void)btn; (void)data;
    /* Spawn ./persk in background */
    if (g_spawn_command_line_async("./persk", NULL)) {
        g_print("[Launcher] Opened Person Management (./persk)\n");
    } else {
...
{
    (void)btn; (void)data;
    if (g_spawn_command_line_async("./newcat", NULL)) {
        g_print("[Launcher] Opened Category Management (./newcat)\n");
    } else {
        GtkWidget *dlg = gtk_message_dialog_new(
            NULL,
            GTK_DIALOG_MODAL,
