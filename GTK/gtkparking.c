...
/*
 * Returns TRUE if 'name' already has a booking that OVERLAPS [req_start, req_end).
 * Used to enforce the one-booking-per-guest-per-timeslot rule.
 * Fills *out with the first conflicting booking found (for the error message).
 *
 * NOTE: a guest MAY have multiple bookings as long as their time windows
 * do not overlap with the requested window.  Checking for *any* existing
 * booking (regardless of time) was the original bug: it prevented a guest
 * from making a second, non-overlapping reservation, and caused the system
 * to falsely report the prior booking as "cancelled" when the confirm step
...
static gboolean guest_has_existing_booking(const char *name, Booking *out,
    ...
    for (int i = 0; i < bcount; i++) {
        ...
        if (!intervals_overlap(req_start, req_end, bs, be)) continue;
...
static void on_guest_check(GtkButton *btn, gpointer data) {
    ...
    if (!strlen(sh_str) || !strlen(sm_str) || !strlen(dur_str)) {
        gtk_label_set_text(GTK_LABEL(guest_result_label),
    ...
    int spot = find_available_spot(req_start, req_end);
    if (spot == -1) {
        char msg[200];
        snprintf(msg, sizeof(msg),
            "⚠  No spot available for %02d:%02d – %02d:%02d.\n"
    ...
    /* Fill pending booking (not saved yet) */
...
static void on_guest_confirm(GtkButton *btn, gpointer data) {
    ...
    /* Guard: reject if the guest already has a booking that overlaps this window */
    {
        ...
        if (guest_has_existing_booking(g_pending_booking.guest_name, &existing,
            ...
            char msg[300];
            snprintf(msg, sizeof(msg),
                "🚫  You already have an overlapping booking:\n"
...
static void go_to_page(GtkButton *btn, gpointer page_name) {
    gtk_stack_set_visible_child_name(GTK_STACK(g_main_stack),
...
static GtkWidget *create_home_page(void) {
    ...
    gtk_label_set_markup(GTK_LABEL(title),
        "<span font='20' weight='bold'>🅿  Parking Management System</span>");
    gtk_widget_set_halign(title, GTK_ALIGN_CENTER);
    gtk_widget_set_margin_top(title, 50);
    gtk_widget_set_margin_bottom(title, 8);
    gtk_box_append(GTK_BOX(outer), title);

    GtkWidget *sub = gtk_label_new("Select a panel to continue");
    gtk_widget_set_halign(sub, GTK_ALIGN_CENTER);
    gtk_widget_set_margin_bottom(sub, 30);
    gtk_box_append(GTK_BOX(outer), sub);

    gtk_box_append(GTK_BOX(outer),
                   gtk_separator_new(GTK_ORIENTATION_HORIZONTAL));

    /* button row */
    GtkWidget *btn_row = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 40);
    gtk_widget_set_halign(btn_row, GTK_ALIGN_CENTER);
    gtk_widget_set_valign(btn_row, GTK_ALIGN_CENTER);
    gtk_widget_set_vexpand(btn_row, TRUE);
    gtk_widget_set_margin_bottom(btn_row, 60);

    /* Guest button */
    GtkWidget *guest_btn_box = gtk_box_new(GTK_ORIENTATION_VERTICAL, 8);
    gtk_widget_set_halign(guest_btn_box, GTK_ALIGN_CENTER);
    GtkWidget *guest_icon = gtk_label_new(NULL);
    gtk_label_set_markup(GTK_LABEL(guest_icon), "<span font='36'>🚗</span>");
    gtk_widget_set_halign(guest_icon, GTK_ALIGN_CENTER);
    GtkWidget *guest_btn = gtk_button_new_with_label("  Guest Panel  ");
    g_signal_connect(guest_btn, "clicked", G_CALLBACK(go_to_page), "guest");
    gtk_box_append(GTK_BOX(guest_btn_box), guest_icon);
    gtk_box_append(GTK_BOX(guest_btn_box), guest_btn);

    /* Admin button */
    GtkWidget *admin_btn_box = gtk_box_new(GTK_ORIENTATION_VERTICAL, 8);
    gtk_widget_set_halign(admin_btn_box, GTK_ALIGN_CENTER);
    GtkWidget *admin_icon = gtk_label_new(NULL);
    gtk_label_set_markup(GTK_LABEL(admin_icon), "<span font='36'>🔧</span>");
    gtk_widget_set_halign(admin_icon, GTK_ALIGN_CENTER);
    GtkWidget *admin_btn = gtk_button_new_with_label("  Admin Panel  ");
    g_signal_connect(admin_btn, "clicked", G_CALLBACK(go_to_page), "admin");
    gtk_box_append(GTK_BOX(admin_btn_box), admin_icon);
    gtk_box_append(GTK_BOX(admin_btn_box), admin_btn);

    gtk_box_append(GTK_BOX(btn_row), guest_btn_box);
    gtk_box_append(GTK_BOX(btn_row), admin_btn_box);
    gtk_box_append(GTK_BOX(outer), btn_row);

    return outer;
}

/* Wrap a panel page with a "← Home" back button at the top */
static GtkWidget *wrap_with_back_button(GtkWidget *page) {
    ...
    GtkWidget *back_btn = gtk_button_new_with_label("← Home");
...
static void activate(GtkApplication *app, gpointer data) {
    ...
    gtk_window_set_default_size(GTK_WINDOW(win), 700, 520);
