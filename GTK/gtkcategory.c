...
int       next_cat_id   = 0;
const char *CAT_CSV = "category.csv";

// ---------- SPLASH / LOGIN ----------
static GtkWidget *cat_login_stack;

// ---------- LOGIN / SPLASH ----------
static GtkWidget *cat_login_stack;  // top-level stack: "login" vs "main"
...
void display_all_guests(Category *tete, GString *out) {
    /* Header – printed once at the top of the display */
...
// LOGIN / SPLASH PAGE
// ============================================================
static void on_cat_enter(GtkButton *btn, gpointer data) {
    gtk_stack_set_visible_child_name(GTK_STACK(cat_login_stack), "main");
}

static GtkWidget* create_cat_login_page(void) {
    GtkWidget *overlay = gtk_overlay_new();

    // Background image
    GtkWidget *bg = gtk_picture_new_for_filename("category_bg.jpg");
    ...
    gtk_widget_add_css_class(card, "cat-login-card");

    GtkWidget *title = gtk_label_new("Category Management");
    gtk_widget_add_css_class(title, "cat-login-title");

    GtkWidget *sub = gtk_label_new("Recognize Every Guest, Make Every Moment Special");
    gtk_label_set_wrap(GTK_LABEL(sub), TRUE);
    gtk_label_set_justify(GTK_LABEL(sub), GTK_JUSTIFY_CENTER);
    gtk_widget_add_css_class(sub, "cat-login-sub");

    GtkWidget *enter_btn = gtk_button_new_with_label("Enter");
    gtk_widget_add_css_class(enter_btn, "cat-login-btn");
    g_signal_connect(enter_btn, "clicked", G_CALLBACK(on_cat_enter), NULL);
...
void activate(GtkApplication *app, gpointer data) {
    ...
    gtk_window_set_title(GTK_WINDOW(win), "Category Manager");
    ...
    gtk_css_provider_load_from_string(css,
        ...
        "  margin-top: 6px;"
        "}"
        ".cat-login-btn:hover { background-color: #e0b84a; }");
    ...
    // Top-level login/splash stack
    cat_login_stack = gtk_stack_new();
    gtk_widget_set_hexpand(cat_login_stack, TRUE);
    gtk_widget_set_vexpand(cat_login_stack, TRUE);
    gtk_stack_add_named(GTK_STACK(cat_login_stack), create_cat_login_page(), "login");
    gtk_stack_add_named(GTK_STACK(cat_login_stack), main_box,                "main");
    gtk_stack_set_visible_child_name(GTK_STACK(cat_login_stack), "login");

    gtk_window_set_child(GTK_WINDOW(win), cat_login_stack);
