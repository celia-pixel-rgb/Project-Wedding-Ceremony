...
void action_add_guest(void) {
    ...
    // each field retries on its own until a valid value is entered
    do {
        read_line("Full Name: ", name, sizeof(name));
        if (strlen(name) < 2) printf("Name too short. Please try again.\n");
    } while (strlen(name) < 2);

    do {
        read_line("Age: ", age_str, sizeof(age_str));
        if (!is_valid_age(age_str)) printf("Invalid age (must be 1-120). Please try again.\n");
    } while (!is_valid_age(age_str));

    read_line("Status (Single/Married/Divorced): ", status, sizeof(status));

    do {
        read_line("Phone (9 digits, start 6-9): ", phone, sizeof(phone));
        if (!is_valid_phone(phone))
            printf("Invalid phone (must be 9 digits starting with 6, 7, 8 or 9). Please try again.\n");
