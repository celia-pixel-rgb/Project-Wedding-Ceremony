...
static const char *GUEST_CSV = "persons.csv";
static const char *PASSWORD  = "group3wed!";
static const char *CAT_CSV   = "category.csv";
static const char *TMP_CSV   = "category_tmp.csv"; /* Temp file for safe saves */
...
static void delete_category(Category **head, int id) {
    ...
    while (curr) {
        if (curr->id == id) {
            ...
            free(curr);
            /* Decrement IDs of all categories that came after the deleted one */
            Category *after = *head;
            while (after) {
                if (after->id > id) after->id--;
                after = after->next;
            }
            if (next_cat_id > 0) next_cat_id--;
            return;
        }
        prev = curr;
        curr = curr->next;
...
static void save_categories_to_csv(void) {
    ...
    if (!f) { printf("Warning: could not save categories.\n"); return; }
    ...
    if (rename(TMP_CSV, CAT_CSV) != 0)  /* rename() returns non-zero on failure */
        printf("Warning: could not finalise categories file.\n");
}
