...
/* ─────────────────────────────────────────────────────────────────────────────
 * INTERNAL HELPER  –  read one line of input safely
 *
 * Reads up to (size-1) characters from stdin into buf, strips the trailing
 * newline, and discards any extra characters left in the buffer.
 * This prevents stale input from corrupting subsequent reads.
 * ───────────────────────────────────────────────────────────────────────────── */
...
void run_guest_menu(const char *guest_name, int num_spots, int max_hours) {
    ...
    while (1) {
        ...
        if (choice == 1) {
            ...
            Booking conflict;
            if (guest_has_overlapping_booking(guest_name, &conflict,
                                              req_start, req_end)) {
                printf("  [!] You already have an overlapping booking:\n");
                printf("      Spot %d  |  %02d:%02d - %02d:%02d\n",
                       conflict.spot_id,
                       conflict.start_hour, conflict.start_min,
                       conflict.end_hour,   conflict.end_min);
                printf("      Please choose a different time window.\n");
                continue;
            }
