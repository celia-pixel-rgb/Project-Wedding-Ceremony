...
// ---------- DISPLAY ----------
GtkWidget *manager_text;
GtkWidget *display_password_entry;

// ---------- UPDATE ----------
GtkWidget *update_id_entry, *update_name, *update_age, *update_status, *update_phone;
...
GtkWidget* create_form_page() {
    ...
    gtk_entry_set_placeholder_text(GTK_ENTRY(status_entry),"VIP / Regular / Guest");
    gtk_entry_set_placeholder_text(GTK_ENTRY(phone_entry),"650123456");
    ...
    gtk_drop_down_set_selected(GTK_DROP_DOWN(parking_dropdown),1);
    g_object_unref(list);
