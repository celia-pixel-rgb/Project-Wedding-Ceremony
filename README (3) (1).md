...
| 🚗 **Parking** | ✅ Complete | Parking spot booking with admin configuration, overlap detection, and guest verification |
...
```
wedding-guest-system/           ← ALL source files are in this flat directory
│
...
│
├── parking.h                 # Parking structs, constants, function prototypes
├── parking.c                 # CLI: admin config, spot booking, overlap detection
│
├── gtkperson.c               # GTK 4 GUI — guest management
├── gtkcategory.c             # GTK 4 GUI — category management
├── gift_gtk03.c              # GTK 4 GUI — gift management
├── gtkparking.c              # GTK 4 GUI — parking management (admin + guest panels)
...
│   ├── parking_spot.csv      # Admin configuration — spot count & max hours
│   └── parking_booking.csv   # Guest reservations — all active bookings
│
...
**What it does:** The Parking module lets the admin configure a pool of numbered parking spots (up to 200) and allows registered guests who have been granted a parking spot in the Person module to book a time slot. The system checks `persons.csv` to verify that a guest exists and has parking enabled (`"Yes"`) before accepting a booking. Multiple bookings can share the same spot as long as their time windows do not overlap — the module performs precise overlap detection that wraps correctly past midnight.

#### Data Structures — `parking.h`

| Symbol | Kind | Description |
|---|---|---|
| `SpotConfig` | `struct` | Admin configuration: `spot_id` (1-based) and `max_hours` — the maximum booking duration allowed |
| `Booking` | `struct` | One guest reservation: `booking_id`, `spot_id`, `guest_name[64]`, `start_hour`, `start_min`, `duration_hours`, `end_hour`, `end_min` |

#### Return Codes — `check_person_parking()`

| Code | Constant | Meaning |
|---|---|---|
| `0` | `PERSON_NOT_FOUND` | Name not found in `persons.csv` |
| `1` | `PERSON_NO_PARKING` | Guest registered but parking field is `"No"` |
| `2` | `PERSON_HAS_PARKING` | Guest registered and parking field is `"Yes"` |

#### CLI Functions — `parking.c`

| Function | Returns | Description |
|---|---|---|
| `to_minutes(h, m)` | `int` | Converts an hour:minute pair to minutes since midnight (e.g. `9:30 → 570`) |
| `add_hours(sh, sm, dur_h, *eh, *em)` | `void` | Adds `dur_h` whole hours to a start time, wrapping correctly past midnight |
| `intervals_overlap(as, ae, bs, be)` | `int` | Returns `1` if the time interval `[as, ae)` overlaps `[bs, be)` (all values in minutes since midnight) |
| `save_spot_config(num_spots, max_hours)` | `void` | Writes the admin's spot count and max-hours setting to `parking_spot.csv` |
| `load_spot_config(*num_spots, *max_hours)` | `int` | Reads spot configuration from `parking_spot.csv`; returns `1` on success, `0` if not found |
| `append_booking(b)` | `void` | Appends a single `Booking` record as a new CSV row to `parking_booking.csv` |
| `next_booking_id()` | `int` | Scans `parking_booking.csv` for the highest ID and returns `max + 1`; returns `1` for an empty file |
| `load_bookings(*out_count)` | `Booking*` | Loads all rows from `parking_booking.csv` into a dynamically allocated array; caller must `free()` the result |
| `cancel_booking_by_id(booking_id)` | `int` | Removes the row matching `booking_id` from `parking_booking.csv`; returns `1` on success |
| `cancel_bookings_for_guest(guest_name)` | `int` | Removes **all** bookings belonging to `guest_name`; returns count removed. Used when parking is revoked in the Person module |
| `check_person_parking(name)` | `int` | Case-insensitive lookup in `persons.csv`; returns one of the three `PERSON_*` codes above |
| `find_available_spot(req_start, req_end, num_spots)` | `int` | Searches spots 1 … `num_spots` for one that does not overlap the requested window; returns the spot number or `-1` if all are taken |
| `guest_has_overlapping_booking(name, *out, req_start, req_end)` | `int` | Returns `1` if the guest already has a booking that overlaps the requested window; fills `*out` with the conflicting booking if so |
| `print_separator()` | `void` | Prints a horizontal dash separator line for CLI formatting |
| `print_all_spots(num_spots)` | `void` | Displays the status of every configured spot: `FREE` or `OCCUPIED` with guest names and time windows |
| `print_guest_bookings(guest_name)` | `void` | Lists all bookings in `parking_booking.csv` for the given guest name |
| `run_admin_menu()` | `void` | 🔑 Password-protected. Interactive menu: configure spots & max hours, view spot occupancy, cancel any booking by ID |
| `run_guest_menu(guest_name, num_spots, max_hours)` | `void` | Interactive guest menu: book a slot, view own bookings, cancel one own booking |

#### GTK Parking GUI — `gtkparking.c`

The GTK version presents two tabbed panels in a single window:

| Panel | Access | Description |
|---|---|---|
| **Admin Panel** | 🔑 Password-protected | Set the number of spots and maximum hours; view real-time spot occupancy status with auto-refresh; cancel any booking by ID |
| **Guest Panel** | Open | Guest enters their name — the app verifies them against `persons.csv` and checks their parking flag. If valid, they choose a start time and duration; the system auto-assigns the first available spot and confirms the booking |

> **GTK Parking — key features:**
> - Guest identity verified against `persons.csv` before any booking is accepted
> - Overlap detection: multiple guests may share a spot as long as their time windows do not conflict
> - Auto-refresh keeps the admin status display up to date via `g_timeout_add()`
> - All data is shared with the CLI version through the same `parking_spot.csv` and `parking_booking.csv` files
> - Up to **200 spots** supported
...
| `gtkcombinedmodules.c` | GTK 4 | Graphical launcher with dark-navy (`#0d1b2e`) + gold (`#c9a84c`) CSS theme. Displays clickable module cards with hover animation. Launches `./gtkperson`, `./gtkcategory`, and `./gift_gtk03` via `system()` |
...
│   │   MODULE     │  persons.csv   │  (verifies guest &   │          │
│   │  (sets       │  (parking flag)│   parking flag,      │          │
│   │ parking flag)│                │   manages bookings)  │          │
│   └──────────────┘                │  parking_spot.csv    │          │
│                                   │  parking_booking.csv │          │
│                                   └──────────────────────┘          │
└──────────────────────────────────────────────────────────────────────┘
...
**Step 4 — Manage Parking Bookings (Parking Module)**
> Once guests are registered and their parking flag is set to `"Yes"` in the Person module, the admin configures the number of available spots and the maximum booking duration. Guests can then book a time slot via the CLI or GTK interface. The system verifies the guest's identity and parking eligibility from `persons.csv` and checks for time-slot conflicts before confirming any booking. If a guest's parking flag is later revoked in the Person module, all of their active bookings are automatically cancelled.
...
<img width="900" height="200" alt="image" src="https://github.com/user-attachments/assets/e7a488ec-cd23-4682-8bbc-1ad7271bdf9a" />
...
Run `ls` (Linux/MSYS2) to confirm files are present:

```
person.c  person.h  category.c  category.h  gift.c  gift.h
parking.c  parking.h  gtkperson.c  gtkcategory.c  gift_gtk03.c
gtkparking.c  gtkcombinedmodules.c  combinedmodules.c  Makefile
```
...
| `make gtkparking` | GTK 4 | GTK 4 parking management GUI |
...
|---|---|---|---|
| GTK graphical launcher *(recommended)* | `./launcher` | GTK 4 | Opens the main portal; from here launch Person, Category or Gift GUI |
...
| GTK parking manager | `./gtkparking` | GTK 4 | Admin configures spots; guests book time slots with overlap detection |
...
| CLI parking manager | `./parking` | Terminal | Admin menu + guest booking menu for parking management |
...
### `csv/parking_spot.csv`

Admin configuration file. Contains a single row defining the lot setup.

```
spot_id,max_hours
```

Each row represents one configurable spot. `spot_id` is 1-based; `max_hours` is the maximum booking duration allowed for that spot.

---

### `csv/parking_booking.csv`

Each row represents one active guest parking reservation.

```
booking_id,spot_id,guest_name,start_hour,start_min,duration_hours,end_hour,end_min
```

`start_hour` / `end_hour` are in 24-hour format (0–23). `end_hour` / `end_min` are computed automatically from start time + duration.
